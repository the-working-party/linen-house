/**
 * layout-marquee-component.js
 *
 * Copied from Horizon's assets/marquee.js.
 * Source: https://github.com/Shopify/horizon
 * Copy taken from: 92ec498eff79cb4b43970bc32e51ce45fa055cc0
 *
 * Class renamed from MarqueeComponent → LayoutMarqueeComponent.
 * Custom element tag changed to layout-marquee-component.
 * All other behaviour preserved exactly.
 *
 * This copy exists because MarqueeComponent is not exported from
 * Horizon's marquee.js (declared without `export`). A follow-up
 * PR to upstream the export would let us replace this copy with
 * an import. See docs/upstream-sync.md.
 */
 ...
 */
class LayoutMarqueeComponent extends Component {
  requiredRefs = ['wrapper', 'content', 'marqueeItems'];

  connectedCallback() {
    super.connectedCallback();

    // Runtime ref guard — surface missing refs clearly in the console
    const missingRefs = this.requiredRefs.filter((ref) => {
      const value = this.refs[ref];
      return value === undefined || value === null || (Array.isArray(value) && value.length === 0);
    });
    if (missingRefs.length > 0) {
      console.error(
        `[LayoutMarqueeComponent] Missing required refs: ${missingRefs.join(', ')}. ` +
        `Ensure the markup contains elements with ref="${missingRefs.join('", ref="')}" attributes.`
      );
      return;
    }
...
if (!customElements.get('layout-marquee-component')) {
  customElements.define('layout-marquee-component', LayoutMarqueeComponent);
}
